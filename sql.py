...
def reparsetx_MP(txhash):
    ...
    tx = dbSelect(
        "select txblocknumber, txseqinblock, txdbserialnum, txstate, txtype from transactions where txhash=%s",
        [txhash])
    if len(tx) == 1:
        tx = tx[0]
    else:
    ...
    if txstate == 'valid':
        addressesintxs = dbSelect(
            "select address, addressrole, protocol, propertyid, balanceavailablecreditdebit, balancereservedcreditdebit, balanceacceptedcreditdebit, balancefrozencreditdebit, linkedtxdbserialnum "
        ...
        for entry in addressesintxs:
            ...
            updateBalance(Address, Protocol, PropertyID, Ecosystem, dbBalanceAvailable, dbBalanceReserved,
                          dbBalanceAccepted, -TxDBSerialNum, dbBalanceFrozen)
...
def reorgRollback(block):
    ...
    txs = dbSelect(
        "select txdbserialnum,txtype,txstate,txblocknumber from transactions where txblocknumber >%s order by txdbserialnum desc",
        [block])
    ...
    for tx in txs:
        ...
        if txstate == 'valid':
            addressesintxs = dbSelect(
                "select address, addressrole, protocol, propertyid, balanceavailablecreditdebit, balancereservedcreditdebit, balanceacceptedcreditdebit,linkedtxdbserialnum "
                "from addressesintxs where txdbserialnum=%s", [TxDbSerialNum])
            ...
            for entry in addressesintxs:
                ...
                # use -1 for txdbserialnum as we don't know what the previous tx that last modified it's balanace was.
                updateBalance(Address, Protocol, PropertyID, Ecosystem, dbBalanceAvailable, dbBalanceReserved,
                              dbBalanceAccepted, -TxDbSerialNum)
                ...
                if Protocol == "Omni":
                    ...
                    if txtype == 20 and Role == 'seller':
                        try:
                            rawtx = json.loads(
                                dbSelect("select txdata from txjson where txdbserialnum=%s", [TxDbSerialNum])[0][0])
                        except TypeError:
                        ...
                        if 'subaction' in rawtx and rawtx['subaction'].lower() == 'cancel':
                            printdebug(("Uncancelling DEx.1 sale", linkedtxdbserialnum, "from transaction",
                                        TxDbSerialNum, Address), 7)
                            # cancellation, undo the cancellation (not sure about the lasttxdbserialnum yet
                            dbExecute(
                                "update activeoffers set offerstate='active',lasttxdbserialnum=-1 where createtxdbserialnum=%s",
                                [linkedtxdbserialnum])
                        ...
                        saletxdbserialnum = \
                            dbSelect("select saletxdbserialnum from offeraccepts where linkedtxdbserialnum=%s",
                                     [TxDbSerialNum])[0][0]
                        dbExecute(
                            "update activeoffers set amountaccepted=amountaccepted+%s::numeric, amountavailable=amountavailable-%s::numeric, "
                            "lasttxdbserialnum=-1 where createtxdbserialnum=%s",
                            (dbBalanceAccepted, dbBalanceAccepted, saletxdbserialnum))
                        ...
                        if Role == 'seller':
                            dbExecute(
                                "update activeoffers set offerstate='active', amountaccepted=amountaccepted+%s::numeric where createtxdbserialnum=%s",
                                (dbBalanceReserved, linkedtxdbserialnum))
                        elif Role == 'buyer':
                            dbExecute(
                                "update offeraccepts set dexstate='unpaid', amountaccepted=amountaccepted-%s::numeric, amountpurchased=amountpurchased+%s::numeric "
                                "where linkedtxdbserialnum=%s",
                                (dbBalanceAvailable, dbBalanceAvailable, linkedtxdbserialnum))

                    elif txtype == 50 or txtype == 51 or txtype == 54:
                        # remove the property and the property history information
                        dbExecute(
                            "delete from smartproperties where createtxdbserialnum=%s and propertyid=%s and protocol=%s",
                            (TxDbSerialNum, PropertyID, Protocol))
                        dbExecute(
                            "delete from propertyhistory where txdbserialnum=%s and propertyid=%s and protocol=%s",
                            (TxDbSerialNum, PropertyID, Protocol))
...
def updateTxStats():
    ...
    if curblock > lastblock:
        ROWS = dbSelect(
            "select count(*) from transactions where txrecvtime >= %s - '1 day'::INTERVAL and txrecvtime <= %s and txdbserialnum>0",
            (btime, btime))
        txs = ROWS[0][0]
...
def checkPending(blocktxs):
    ...
    pendingtxs = dbSelect(
        "select txhash,txdbserialnum,protocol,extract(epoch from txrecvtime) from transactions where txstate='pending' and txdbserialnum < 0")
    ...
    for rawtx in pendingList['result']:
        try:
            ...
            if txtype in [4]:
                ...
                dbExecute(
                    "insert into addressesintxs (address,propertyid,protocol,txdbserialnum,addresstxindex,addressrole,balanceavailablecreditdebit,balanceacceptedcreditdebit) "
                    "values(%s,%s,%s,%s,%s,%s,%s,%s)",
                    (address, propertyid, protocol, txdbserialnum, addresstxindex, saddressrole, sendamount, sbacd))
                ...
                if receiver != "":
                    ...
                    dbExecute(
                        "insert into addressesintxs (address,propertyid,protocol,txdbserialnum,addresstxindex,addressrole,balanceavailablecreditdebit,balanceacceptedcreditdebit) "
                        "values(%s,%s,%s,%s,%s,%s,%s,%s)",
                        (address, propertyid, protocol, txdbserialnum, addresstxindex, raddressrole, recvamount, rbacd))
                    # update pending balance
            ...
            dbExecute(
                "insert into transactions (txhash,protocol,txdbserialnum,txtype,txversion) values(%s,%s,%s,%s,%s)",
                (txhash, protocol, txdbserialnum, txtype, txversion))

            # store decoded omni data until tx confirms
            dbExecute("insert into txjson (txdbserialnum, protocol, txdata) values (%s,%s,%s)",
                      (txdbserialnum, protocol, json.dumps(rawtx)))
...
def expireAccepts(Block):
    ...
    if Block < 0:
        ...
        expiring = dbSelect(
            "select oa.amountaccepted, oa.saletxdbserialnum, ao.offerstate from offeraccepts as oa, activeoffers as ao "
    ...
    for offer in expiring:
        ...
        if salestate == 'replaced' or salestate == 'cancelled':
            printdebug(
                ("Found replaced/expired sale", saletxserialnum, "for expiring accept with amount", amountaccepted), 4)
...
def updateAccept(Buyer, Seller, AmountBought, PropertyIDBought, TxDBSerialNum):
    ...
    accept = dbSelect(
        "select oa.amountaccepted, oa.amountpurchased, ao.amountaccepted, ao.amountavailable, ao.offerstate, oa.saletxdbserialnum, oa.linkedtxdbserialnum "
...
def offerAccept(rawtx, TxDBSerialNum, Block):
    ...
    saleinfo = dbSelect(
        "select createtxdbserialnum,timelimit,amountaccepted,amountavailable from activeoffers where seller=%s and offerstate='active'"
    ...
    if valid:
        ...
        dbExecute(
            "update activeoffers set amountaccepted=%s, amountavailable=%s, lasttxdbserialnum=%s  where  seller=%s and offerstate='active' and propertyidselling=%s and propertyiddesired=%s",
    ...
    # insert the offer
    dbExecute(
        "insert into offeraccepts (buyer, amountaccepted, linkedtxdbserialnum, saletxdbserialnum, block, dexstate, expireblock, expiredstate) "
        "values(%s,%s,%s,%s,%s,%s,%s,%s)",
...
def updatedex(rawtx, TxDBSerialNum, Protocol):
    ...
    remaining = dbSelect(
        "select amountavailable,createtxdbserialnum from activeoffers where seller=%s and offerstate='active' and propertyiddesired=%s and propertyidselling=%s",
    ...
    if subaction.lower() == 'cancel':
        ...
        dbExecute(
            "update activeoffers set offerstate=%s, LastTxDBSerialNum=%s where seller=%s and offerstate='active' and propertyiddesired=%s and propertyidselling=%s",
        ...
        if amount is not None:
            printdebug(("replacing old sale", createtxdbserialnum, "with amount remaining", amount, "with newsale",
                        TxDBSerialNum), 4)
            ...
            updateBalance(Address, Protocol, propertyidselling, Ecosystem, BalanceAvailable, BalanceReserved,
                          BalanceAccepted, TxDBSerialNum)
...
def insertMatch(rawtrade, match, TxDBSerialNum):
    ...
    dbExecute(
        "insert into matchedtrades (txdbserialnum,txhash,propertyidsold,propertyidreceived,amountsold,amountreceived,block,tradingfee,matchedtxhash) "
        "values (%s,%s,%s,%s,%s,%s,%s,%s,%s)",
        (TxDBSerialNum, txhash, propertyidsold, propertyidreceived, amountsold, amountreceived, block, tradingfee,
         matchedtxhash))


def updatemarketvolume():
    ROWS = dbSelect(
        "select COALESCE(sum(balanceavailablecreditdebit),0) from transactions tx, addressesintxs atx where "
...
def updatemarkets(propertyidselling, propertyiddesired, TxDBSerialNum, rawtx):
    ...
    UP = dbSelect(
        "select amountdesired, amountavailable, totalselling, unitprice from activeoffers where offerstate='active' and propertyidselling=%s "
        "and propertyiddesired=%s and amountavailable!=totalselling order by unitprice asc",
    ...
    if len(UP) > 0:
        for offer in UP:
            ...
            efup = decimal.Decimal((round(
                (decimal.Decimal(remaindesired) / decimal.Decimal(availselling)) * decimal.Decimal(
                    1e8)))) / decimal.Decimal(1e8)
            if efup < unitprice or unitprice in [0, None]:
                unitprice = efup

    lastprice = dbSelect("select unitprice from markets where propertyidselling=%s and propertyiddesired=%s ",
                         (propertyidselling, propertyiddesired))
    if len(lastprice) > 0:
...
def syncAddress(Address, Protocol):
    ...
    for property in baldata:
        ...
        if len(rows) == 0:
            ...
            dbExecute(
                "UPDATE AddressBalances set BalanceAvailable=%s, BalanceReserved=%s, BalanceAccepted=%s where address=%s and PropertyID=%s",
...
def resetbalances_MP():
    ...
    for PropertyID in listlookup:
        ...
        dbExecute(
            "update addressbalances set BalanceAvailable=0, BalanceReserved=0, BalanceAccepted=0, BalanceFrozen=0 where protocol=%s and propertyid=%s",
            (Protocol, PropertyID))
        ...
        for addr in bal_data['result']:
            ...
            if len(rows) == 0:
                ...
                # address in database update
                dbExecute(
                    "UPDATE AddressBalances set BalanceAvailable=%s, BalanceReserved=%s, BalanceAccepted=%s, BalanceFrozen=%s where address=%s and PropertyID=%s",
        ...
        for addr in bal_data['result']:
            ...
            if len(rows) == 0 and (BalanceAvailable != 0 or BalanceReserved != 0 or BalanceAccepted != 0):
                ...
                item = [{'Address': Address,
                         'bal': {'Status': 'Missing', 'PropertyID': PropertyID, 'BalanceAvailable': BalanceAvailable,
                                 'BalanceReserved': BalanceReserved, 'BalanceAccepted': BalanceAccepted}}]
                ...
                if BalanceAvailable != dbBalanceAvailable:
                    item = [{'Address': Address, 'bal': {'Status': 'Mismatch', 'PropertyID': PropertyID,
                                                         'BalanceAvailable': BalanceAvailable,
                                                         'dbBalanceAvailable': dbBalanceAvailable,
                                                         'dbBalanceReserved': dbBalanceReserved,
                                                         'BalanceReserved': BalanceReserved,
                                                         'dbBalanceAccepted': dbBalanceAccepted,
                                                         'BalanceAccepted': BalanceAccepted}}]
                    # add the missing/incorrect item to our list to return
                    try:
                        retval[PropertyID] = retval[PropertyID] + item
                    except KeyError:
...
def updateBalance(Address, Protocol, PropertyID, Ecosystem, BalanceAvailable, BalanceReserved, BalanceAccepted,
                  LastTxDBSerialNum, BalanceFrozen=0):
    printdebug("Starting updateBalance:", 4)
    printdebug(
        "Address, Protocol, PropertyID, Ecosystem, BalanceAvailable, BalanceReserved, BalanceAccepted, TxDBSerialNum",
        4)
    printdebug((Address, Protocol, PropertyID, Ecosystem, BalanceAvailable, BalanceReserved, BalanceAccepted,
                LastTxDBSerialNum, "\n"), 4)
    ...
    if LastTxDBSerialNum is not None and LastTxDBSerialNum < 0:
        txrow = dbSelect(
            "select max(atx.txdbserialnum) from addressesintxs atx, transactions tx where atx.txdbserialnum=tx.txdbserialnum and "
    ...
    if len(rows) == 0:
        ...
        dbExecute("INSERT into AddressBalances "
                  ...
                  "VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                  (Address, Protocol, PropertyID, Ecosystem, BalanceAvailable, BalanceReserved, BalanceAccepted,
                   BalanceFrozen, LastTxDBSerialNum))
    else:
        # address in database update
        ...
        dbExecute(
            "UPDATE AddressBalances set BalanceAvailable=%s, BalanceReserved=%s, BalanceAccepted=%s, BalanceFrozen=%s, LastTxDBSerialNum=%s where address=%s and PropertyID=%s and Protocol=%s",
            (BalanceAvailable, BalanceReserved, BalanceAccepted, BalanceFrozen, LastTxDBSerialNum, Address, PropertyID,
             Protocol))
...
def expireCrowdsales(BlockTime, Protocol):
    ...
    if BlockTime < 0:
        ...
        expired = dbSelect("select propertyid from smartproperties as sp inner join transactions as tx on "
                           ...
                           "cast(propertydata::json->>'endedtime' as numeric) >= %s and propertydata::json->>'active'='false'",
                           (Protocol, BlockTime))
        ...
        expiring = dbSelect(
            "select propertyid from smartproperties as sp inner join transactions as tx on (sp.createtxdbserialnum=tx.txdbserialnum) "
...
def updateProperty(PropertyID, Protocol, LastTxDBSerialNum=None):
    ...
    if PropertyID == 0:
        rawprop = {"name": "BTC", "blocktime": 1231006505,
            ...
            if addedissuertokens > 0:
                if reorg:
                    ...
                    if rawprop['active']:
                        ...
                        updateBalance(issuer, Protocol, PropertyID, Ecosystem, addedissuertokens, 0, 0,
                                      LastTxDBSerialNum)
        except Exception:
...
def insertProperty(rawtx, Protocol, PropertyID=None):
    ...
    if rawtx['result']['valid']:
        ...
        if len(ROWS) > 0:
            ...
            dbExecute(
                "update smartproperties set Issuer=%s, Ecosystem=%s, CreateTxDBSerialNum=%s, LastTxDBSerialNum=%s, "
            ...
            dbExecute("insert into PropertyHistory (Protocol, PropertyID, TxDBSerialNum) Values(%s, %s, %s)",
                      (Protocol, PropertyID, LastTxDBSerialNum))
        else:
...
def getFlags(Protocol, name, data, url, PropertyID):
    ...
    try:
        if len(name) > 0:
            sname = '%' + str(name) + '%'
            rname = dbSelect(
                "select count(*) from smartproperties where Protocol=%s and (LOWER(PropertyName) like LOWER(%s) or LOWER(PropertyData->>'data') like LOWER(%s) or LOWER(PropertyData->>'url') like LOWER(%s))",
                (Protocol, sname, sname, sname))
        else:
            rname = [[0]]
        if len(data) > 0:
            sdata = '%' + str(data) + '%'
...
def updateAddrStats(Address, Protocol, TxDBSerialNum, Block):
    ...
    dbExecute("with upsert as "
              ...
              (TxDBSerialNum, Block, Address, Protocol, TxDBSerialNum, Address, Protocol, Address, Protocol,
               TxDBSerialNum, Block))
    ...
    if Protocol == "Bitcoin":
            ...
            if 'addresses' in output['scriptPubKey']:
                ...
                for addr in output['scriptPubKey']['addresses']:
                    dbExecute("insert into addressesintxs "
                              "(Address, PropertyID, Protocol, TxDBSerialNum, AddressTxIndex, AddressRole, BalanceAvailableCreditDebit)"
                              "values(%s, %s, %s, %s, %s, %s, %s)",
                              (addr, PropertyID, Protocol, TxDBSerialNum, AddressTxIndex, AddressRole,
                               BalanceAvailableCreditDebit))
                    updateBalance(addr, Protocol, PropertyID, Ecosystem, BalanceAvailableCreditDebit, 0, 0,
                                  TxDBSerialNum)
        ...
        for _input in rawtx['result']['vin']:
            ...
            if 'txid' in _input:
                ...
                BalanceAvailableCreditDebit = int(
                    decimal.Decimal(prevtx['result']['vout'][_input['vout']]['value']) * decimal.Decimal(
                        "1e8") * decimal.Decimal(-1))
                ...
                for addr in prevtx['result']['vout'][_input['vout']]['scriptPubKey']['addresses']:
                    dbExecute("insert into addressesintxs "
                              ...
                              "values(%s, %s, %s, %s, %s, %s, %s, %s)",
                              (addr, PropertyID, Protocol, TxDBSerialNum, LinkedTxDBSerialNum, AddressTxIndex,
                               AddressRole, BalanceAvailableCreditDebit))
                    updateBalance(addr, Protocol, PropertyID, Ecosystem, BalanceAvailableCreditDebit, 0, 0,
                                  TxDBSerialNum)
                    updateAddrStats(addr, Protocol, TxDBSerialNum, Block)
                AddressTxIndex += 1
        ...
        if txtype == 0:
            ...
            dbExecute("insert into addressesintxs "
                      ...
                      "values(%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                      (Address, PropertyID, Protocol, TxDBSerialNum, AddressTxIndex, AddressRole,
                       BalanceAvailableCreditDebit, BalanceReservedCreditDebit, BalanceAcceptedCreditDebit))
            if Valid:
                updateBalance(Address, Protocol, PropertyID, Ecosystem, BalanceAvailableCreditDebit,
                              BalanceReservedCreditDebit, BalanceAcceptedCreditDebit, TxDBSerialNum)
        ...
        # Restricted Send does nothing yet?
            ...
            if Valid:
                ...
                dbExecute("insert into addressesintxs "
                          "(Address, PropertyID, Protocol, TxDBSerialNum, AddressTxIndex, AddressRole, BalanceAvailableCreditDebit, BalanceReservedCreditDebit, BalanceAcceptedCreditDebit)"
                          "values(%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                          (Address, feeid, Protocol, TxDBSerialNum, 1, AddressRole, stofee, BalanceReservedCreditDebit,
                           BalanceAcceptedCreditDebit))
                updateBalance(Address, Protocol, feeid, feeEco, stofee, BalanceReservedCreditDebit,
                              BalanceAcceptedCreditDebit, TxDBSerialNum)
                ...
                for recv in rawsto['result']['recipients']:
                    ...
                    dbExecute("insert into addressesintxs "
                              "(Address, PropertyID, Protocol, TxDBSerialNum, AddressTxIndex, AddressRole, BalanceAvailableCreditDebit, BalanceReservedCreditDebit, BalanceAcceptedCreditDebit)"
                              "values(%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                              (rAddress, PropertyID, Protocol, TxDBSerialNum, txindex, AddressRole, rBalance,
                               BalanceReservedCreditDebit, BalanceAcceptedCreditDebit))

                    updateBalance(rAddress, Protocol, PropertyID, Ecosystem, rBalance, BalanceReservedCreditDebit,
                                  BalanceAcceptedCreditDebit, TxDBSerialNum)
                    txindex += 1
            ...
            for send in rawtx['result']['subsends']:
                ...
                dbExecute("insert into addressesintxs "
                          "(Address, PropertyID, Protocol, TxDBSerialNum, AddressTxIndex, AddressRole, BalanceAvailableCreditDebit, BalanceReservedCreditDebit, BalanceAcceptedCreditDebit)"
                          "values(%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                          (RecvAddress, PropertyID, Protocol, TxDBSerialNum, AddressTxIndex, RecvRole,
                           BalanceAvailableCreditDebit, BalanceReservedCreditDebit, BalanceAcceptedCreditDebit))
                ...
                if Valid:
                    ...
                    updateBalance(RecvAddress, Protocol, PropertyID, Ecosystem, BalanceAvailableCreditDebit,
                                  BalanceReservedCreditDebit, BalanceAcceptedCreditDebit, TxDBSerialNum)
            ...
            for payment in rawtx['result']['purchases']:
                ...
                dbExecute("insert into addressesintxs "
                          ...
                          (Seller, PropertyIDBought, Protocol, TxDBSerialNum, AddressTxIndex, AddressRole,
                           BalanceAvailableCreditDebit, BalanceReservedCreditDebit, BalanceAcceptedCreditDebit,
                           saletxdbserialnum))

                if Valid:
                    updateBalance(Seller, Protocol, PropertyIDBought, Ecosystem, BalanceAvailableCreditDebit,
                                  BalanceReservedCreditDebit, BalanceAcceptedCreditDebit, TxDBSerialNum)
                ...
                dbExecute("insert into addressesintxs "
                          "(Address, PropertyID, Protocol, TxDBSerialNum, AddressTxIndex, AddressRole, BalanceAvailableCreditDebit, BalanceReservedCreditDebit, BalanceAcceptedCreditDebit, linkedtxdbserialnum)"
                          "values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                          (Buyer, PropertyIDBought, Protocol, TxDBSerialNum, AddressTxIndex, AddressRole,
                           BalanceAvailableCreditDebit, BalanceReservedCreditDebit, BalanceAcceptedCreditDebit,
                           offertxdbserialnum))

                if Valid:
                    updateBalance(Buyer, Protocol, PropertyIDBought, Ecosystem, BalanceAvailableCreditDebit,
                                  BalanceReservedCreditDebit, BalanceAcceptedCreditDebit, TxDBSerialNum)
            ...
            dbExecute("insert into addressesintxs "
                      "(Address, PropertyID, Protocol, TxDBSerialNum, AddressTxIndex, AddressRole, BalanceAvailableCreditDebit, BalanceReservedCreditDebit, BalanceAcceptedCreditDebit)"
                      "values(%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                      (Address, PropertyIdForSale, Protocol, TxDBSerialNum, AddressTxIndex, AddressRole,
                       BalanceAvailableCreditDebit, BalanceReservedCreditDebit, BalanceAcceptedCreditDebit))

            if Valid:
                # Update our DEx/balance tables if its a valid dex sale
                updateBalance(Address, Protocol, PropertyIdForSale, Ecosystem, BalanceAvailableCreditDebit,
                              BalanceReservedCreditDebit, BalanceAcceptedCreditDebit, TxDBSerialNum)
                rawtrade = gettrade(TxHash)
                ...
                for match in rawtrade['result']['matches']:
                    ...
                    if txblock == match['block'] and matchtxdbserialnum < TxDBSerialNum:
                        ...
                        dbExecute("insert into addressesintxs "
                                  ...
                                  "values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                                  (Address, PropertyIdForSale, Protocol, TxDBSerialNum, AddressTxIndex, AddressRole,
                                   BalanceAvailableCreditDebit, -amountsold, BalanceAcceptedCreditDebit,
                                   matchtxdbserialnum))
                        ...
                        dbExecute("insert into addressesintxs "
                                  "(Address, PropertyID, Protocol, TxDBSerialNum, AddressTxIndex, AddressRole, BalanceAvailableCreditDebit, BalanceReservedCreditDebit, BalanceAcceptedCreditDebit, linkedtxdbserialnum)"
                                  "values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                                  (BuyerAddress, PropertyIdForSale, Protocol, TxDBSerialNum, AddressTxIndex, BuyerRole,
                                   amountsold, BalanceReservedCreditDebit, BalanceAcceptedCreditDebit,
                                   matchtxdbserialnum))

                        updateBalance(Address, Protocol, PropertyIdForSale, Ecosystem, BalanceAvailableCreditDebit,
                                      -amountsold, BalanceAcceptedCreditDebit, TxDBSerialNum)
                        updateBalance(BuyerAddress, Protocol, PropertyIdForSale, Ecosystem, amountsold,
                                      BalanceReservedCreditDebit, BalanceAcceptedCreditDebit, TxDBSerialNum)
                        ...
                        dbExecute("insert into addressesintxs "
                                  ...
                                  "values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                                  (
                                      BuyerAddress, PropertyIdDesired, Protocol, TxDBSerialNum, AddressTxIndex,
                                      AddressRole,
                                      BalanceAvailableCreditDebit, -amountreceived, BalanceAcceptedCreditDebit,
                                      matchtxdbserialnum))
                        ...
                        dbExecute("insert into addressesintxs "
                                  "(Address, PropertyID, Protocol, TxDBSerialNum, AddressTxIndex, AddressRole, BalanceAvailableCreditDebit, BalanceReservedCreditDebit, BalanceAcceptedCreditDebit, linkedtxdbserialnum)"
                                  "values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                                  (Address, PropertyIdDesired, Protocol, TxDBSerialNum, AddressTxIndex, BuyerRole,
                                   amountreceived, BalanceReservedCreditDebit, BalanceAcceptedCreditDebit,
                                   matchtxdbserialnum))

                        updateBalance(Address, Protocol, PropertyIdDesired, Ecosystem, amountreceived,
                                      BalanceReservedCreditDebit, BalanceAcceptedCreditDebit, TxDBSerialNum)
                        # in this instance the buyer is actually selling the 2nd property so we deduct from their reserved balance
                        updateBalance(BuyerAddress, Protocol, PropertyIdDesired, Ecosystem, BalanceAvailableCreditDebit,
                                      -amountreceived, BalanceAcceptedCreditDebit, TxDBSerialNum)
            ...
            if getdivisible_MP(PropertyID):
                ...
                BalanceAvailableCreditDebit = int(
                    decimal.Decimal(str(rawtx['result']['purchasedtokens'])) * decimal.Decimal(1e8))
            else:
            ...
            if IssuerCreditDebit > 0:
                dbExecute("insert into addressesintxs "
                          "(Address, PropertyID, Protocol, TxDBSerialNum, AddressTxIndex, AddressRole, BalanceAvailableCreditDebit, BalanceReservedCreditDebit, BalanceAcceptedCreditDebit)"
                          "values(%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                          (Address, PropertyID, Protocol, TxDBSerialNum, AddressTxIndex, AddressRole, IssuerCreditDebit,
                           BalanceReservedCreditDebit, BalanceAcceptedCreditDebit))
                if Valid:
                    updateBalance(Address, Protocol, PropertyID, Ecosystem, IssuerCreditDebit,
                                  BalanceReservedCreditDebit, BalanceAcceptedCreditDebit, TxDBSerialNum)

            # Participating in crowdsale, update smartproperty history table with the propertyid bought
            dbExecute("insert into PropertyHistory (Protocol, PropertyID, TxDBSerialNum) Values(%s, %s, %s)",
                      (Protocol, PropertyID, TxDBSerialNum))
        ...
        # promote crowdsale does what?
            ...
            if Receiver is not None and Receiver != Address and Receiver != "":
                dbExecute("insert into addressesintxs "
                          "(Address, PropertyID, Protocol, TxDBSerialNum, AddressTxIndex, AddressRole, BalanceAvailableCreditDebit, BalanceReservedCreditDebit, BalanceAcceptedCreditDebit)"
                          "values(%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                          (Receiver, PropertyID, Protocol, TxDBSerialNum, AddressTxIndex, ReceiveRole,
                           BalanceAvailableCreditDebit, BalanceReservedCreditDebit, BalanceAcceptedCreditDebit))
                if Valid:
                    updateBalance(Receiver, Protocol, PropertyID, Ecosystem, BalanceAvailableCreditDebit,
                                  BalanceReservedCreditDebit, BalanceAcceptedCreditDebit, TxDBSerialNum)
            ...
            ROWS = dbSelect(
                "select BalanceAvailable,BalanceFrozen from addressbalances where address=%s and propertyid=%s",
                (Address, PropertyID))
            ...
            dbExecute("insert into addressesintxs "
                      ...
                      (Address, PropertyID, Protocol, TxDBSerialNum, AddressTxIndex, AddressRole,
                       BalanceAvailableCreditDebit, BalanceReservedCreditDebit, BalanceAcceptedCreditDebit,
                       BalanceFrozenCreditDebit, linkedtxdbserialnum))
...
def insertBlock(block_data, Protocol, block_height, txcount):
    ...
    dbExecute("INSERT into Blocks"
              ...
              "VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
              (BlockNumber, Protocol, BlockTime, version, blockhash, prevblockhash, merkleroot, bits, nonce, size,
               txcount))
