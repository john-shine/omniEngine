...
for x in txs:
    ...
    for _x in addressbalances:
        ...
        if _x[2] is None:
            ...
            dbBalanceReserved = _x[2]
        if _x[3] is None:
            dbBalanceAccepted = 0
        else:
            dbBalanceAccepted = _x[3]
        balance[_x[0]] = {'available': dbBalanceAvailable, 'reserved': dbBalanceReserved, 'accepted': dbBalanceAccepted}

    # get all records from addressintxs that affected our propertyid after the send to owner block was processed
    changedata = dbSelect(
        "select address, balanceavailablecreditdebit, balancereservedcreditdebit, balanceacceptedcreditdebit "
