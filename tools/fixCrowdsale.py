...
for tx in txtofix:
    ...
    if type == -51:
        ...
        if IssuerCreditDebit > 0:
            dbExecute(
                "update addressesintxs set AddressRole=%s, BalanceAvailableCreditDebit=%s where Address=%s and propertyid=%s and txdbserialnum=%s and protocol=%s",
                (AddressRole, IssuerCreditDebit, Address, PropertyID, TxDBSerialNum, Protocol))
        else:
            # remove old 'bad' entry
            dbExecute(
                "delete from addressesintxs where Address=%s and propertyid=%s and txdbserialnum=%s and protocol=%s",
                (Address, PropertyID, TxDBSerialNum, Protocol))
