...
def dumpblocks_csv(csvwb, block_data, Protocol, block_height, txcount):
    ...
    row = {'BlockNumber': block_height, 'Protocol': Protocol, 'BlockTime': BlockTime, 'Version': version,
           'BlockHash': blockhash,
           'PrevBlock': prevblockhash, 'MerkleRoot': merkleroot, 'Bits': bits, 'Nonce': nonce, 'Size': size,
           'TxCount': txcount}
    csvwb.writerow(row)
...
def dumptxaddr_csv(csvwb, rawtx, Protocol, TxDBSerialNum):
    ...
    if Protocol == "Bitcoin":
        ...
        for output in rawtx['result']['vout']:
            ...
            if 'addresses' in output['scriptPubKey']:
                ...
                for addr in output['scriptPubKey']['addresses']:
                    row = {'Address': addr, 'PropertyID': PropertyID, 'Protocol': Protocol,
                           'TxDBSerialNum': TxDBSerialNum, 'AddressTxIndex': AddressTxIndex,
        ...
        for input in rawtx['result']['vin']:
            ...
            if 'txid' in input:
                ...
                BalanceAvailableCreditDebit = int(
                    decimal.Decimal(prevtx['result']['vout'][input['vout']]['value']) * decimal.Decimal(
        ...
        if type == 0:
            ...
            # debit the sender
            row = {'Address': Address, 'PropertyID': PropertyID, 'Protocol': Protocol, 'TxDBSerialNum': TxDBSerialNum,
                   'AddressTxIndex': AddressTxIndex,
                   'AddressRole': AddressRole, 'BalanceAvailableCreditDebit': value_neg,
                   'BalanceReservedCreditDebit': BalanceReservedCreditDebit,
                   'BalanceAcceptedCreditDebit': BalanceAcceptedCreditDebit}
            csvwb.writerow(row)
        ...
        # return
            ...
            for payment in rawtx['result']['purchases']:
                ...
                row = {'Address': Receiver, 'PropertyID': PropertyIDBought, 'Protocol': Protocol,
                       'TxDBSerialNum': TxDBSerialNum, 'AddressTxIndex': AddressTxIndex,
                       'AddressRole': AddressRole, 'BalanceAvailableCreditDebit': BalanceAvailableCreditDebit,
                       'BalanceReservedCreditDebit': BalanceReservedCreditDebit,
                       'BalanceAcceptedCreditDebit': BalanceAcceptedCreditDebit}
                ...
                row = {'Address': Sender, 'PropertyID': PropertyIDBought, 'Protocol': Protocol,
                       'TxDBSerialNum': TxDBSerialNum, 'AddressTxIndex': AddressTxIndex,
                       'AddressRole': AddressRole, 'BalanceAvailableCreditDebit': BalanceAvailableCreditDebit,
                       'BalanceReservedCreditDebit': BalanceReservedCreditDebit,
                       'BalanceAcceptedCreditDebit': BalanceAcceptedCreditDebit}
            ...
            if cstx['result']['percenttoissuer'] > 0:
                if getdivisible_MP(PropertyID):
                    BalanceAvailableCreditDebit = int(decimal.Decimal(rawtx['result']['amount']) * decimal.Decimal(
                        cstx['result']['tokensperunit']) * (decimal.Decimal(
                        cstx['result']['percenttoissuer']) / decimal.Decimal(100)) * decimal.Decimal(1e8))
                else:
                    BalanceAvailableCreditDebit = int(decimal.Decimal(rawtx['result']['amount']) * decimal.Decimal(
                        cstx['result']['tokensperunit']) * decimal.Decimal(
                        (cstx['result']['percenttoissuer']) / decimal.Decimal(100)))
            row = {'Address': Address, 'PropertyID': PropertyID, 'Protocol': Protocol, 'TxDBSerialNum': TxDBSerialNum,
                   'AddressTxIndex': AddressTxIndex,
                   'AddressRole': AddressRole, 'BalanceAvailableCreditDebit': BalanceAvailableCreditDebit,
                   'BalanceReservedCreditDebit': BalanceReservedCreditDebit,
...
def dumptx_csv(csvwb, rawtx, Protocol, block_height, seq, dbserialnum):
    ...
    row = {'TxHash': TxHash, 'Protocol': Protocol, 'TxDBSerialNum': TxDBSerialNum, 'TxType': TxType,
           'TxVersion': TxVersion, 'Ecosystem': Ecosystem,
