...
def getfixedprice(desiredvalue, base):
    ...
    ROWS = dbSelect(
        "select rate1for2 from exchangerates where protocol1='Fiat' and propertyid1=%s and protocol2='Bitcoin' and propertyid2=0 "
...
def upsertRate(protocol1, propertyid1, protocol2, propertyid2, rate, source, timestamp=None):
    ...
    if timestamp is None:
        ...
        dbExecute("with upsert as "
                  ...
                  "where not exists (select * from upsert)",
                  (protocol1, propertyid1, protocol2, propertyid2, rate, source, protocol1, propertyid1, protocol2,
                   propertyid2, rate, source))
    else:
        # if we have a record with the same exchangerate / source just update timestamp, otherwise insert new record
        dbExecute("with upsert as "
                  ...
                  "where not exists (select * from upsert)",
                  (timestamp, protocol1, propertyid1, protocol2, propertyid2, rate, source, protocol1, propertyid1,
                   protocol2, propertyid2, rate, source, timestamp))
...
def updateOMNISP():
    try:
        ...
        ROWS = dbSelect(
            "select propertyid from smartproperties where propertyid >0 and Protocol='Omni' order by propertyid")
