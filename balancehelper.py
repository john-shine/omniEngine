...
def get_balancedata(address):
    ...
    for balrow in ROWS:
        ...
        sym_t = ('BTC' if cID == '0' else (
            'OMNI' if cID == '1' else ('T-OMNI' if cID == '2' else 'SP' + cID)))  # symbol template
        # 1 = new indivisible property, 2=new divisible property (per spec)
        divi = True if int(balrow[1]) == 2 else False
        res = {'symbol': sym_t, 'divisible': divi, 'id': cID, 'pendingpos': str(long(balrow[3])),
               'pendingneg': str(long(balrow[4]))}
        if cID == '0':
